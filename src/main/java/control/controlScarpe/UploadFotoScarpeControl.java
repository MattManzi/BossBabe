...
@WebServlet("/Admin/UploadFotoScarpeControl")
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 2, // 2MB
maxFileSize = 1024 * 1024 * 10, // 10MB
maxRequestSize = 1024 * 1024 * 50) // 50MB

public class UploadFotoScarpeControl extends HttpServlet{
	private static final long serialVersionUID = 1L;
	
	public UploadFotoScarpeControl() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("Foto sotto scarpe");
		...
		for (Part part : request.getParts()){
			...
			if (fileName != null && !fileName.equals("") && (copertina.equals("0"))) {
				...
				try {
					...
					ScarpeModel.updateCopertina(codice ,savePath + File.separator + fileName);
			...
			if (fileName != null && !fileName.equals("")&& (copertina.equals("2"))) {
				...
				try {
					System.out.println("arivato all updata");
					ScarpeModel.updateCopertina2(codice ,savePath + File.separator + fileName);
				} catch (SQLException sqlException) {
					System.out.println(sqlException);
				}
			}
			...
			if (fileName != null && !fileName.equals("") && (copertina.equals("3"))) {
				...
				try {
					System.out.println("arivato all updata");
					ScarpeModel.updateCopertina3(codice ,savePath + File.separator + fileName);
				} catch (SQLException sqlException) {
					System.out.println(sqlException);
				}
			}
